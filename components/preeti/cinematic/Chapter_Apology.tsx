      ...
      <AnimatePresence>
        {!isBlooming && (
          <motion.p
            ...
            className="absolute bottom-12 text-slate-400 font-sans tracking-widest text-sm uppercase z-20 pointer-events-none"
          >
            Click the closed flower to let it bloom
          </motion.p>
        )}
        {isBlooming && !isFinished && revealedIndex >= 0 && (
          <motion.p
            key="continue-instruction"
            initial={{ opacity: 0 }}
            animate={{ opacity: [0.5, 1, 0.5] }}
            transition={{ duration: 2, repeat: Infinity, delay: 3 }}
            className="absolute bottom-12 text-white/50 font-sans tracking-widest text-sm uppercase z-20 pointer-events-none"
          >
